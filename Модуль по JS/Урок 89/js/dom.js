...
export const initTodoHandlers = (todos) => {
    ...
    todosElement.addEventListener("click", event => {
        ...
        if (event.target.matches(".button-complete")) { // element.matches(селектор) проверяет, соответствует ли элемент указанному CSS-селектору. Возвращает true или false.
